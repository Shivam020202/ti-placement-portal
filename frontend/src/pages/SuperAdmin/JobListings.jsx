...
const JobListings = () => {
  ...
  const { data: jobsData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        // First try getting the job page data
        ...
        return {
          ...
          metrics: {
            ...
            liveListings: mockJobs.filter(job => job.status === "approved").length,
            Companies: 3
          }
        };
      }
  ...
  const allJobs = React.useMemo(() => {
    console.log('Computing allJobs with jobsData:', jobsData);
    if (!jobsData) return [];
    const jobs = [...(jobsData.activeJobs || []), ...(jobsData.inactiveJobs || [])];
    console.log('Combined Jobs:', jobs);
    return jobs;
  }, [jobsData]);

  const sortedJobs = React.useMemo(() => {
    console.log('Sorting jobs:', allJobs);
    ...
    <DashboardLayout>
      <div className="w-full h-[80vh] overflow-hidden bg-white rounded-xl p-7 flex flex-col">
        <div className="flex justify-between items-center mb-6">
          <h1 className="font-semibold text-2xl">Listing Requests</h1>
          <div className="flex items-center px-2 rounded-lg bg-dark text-white">
            <RiFilter3Line size={20} />
            <select
              className="select bg-dark focus:outline-none"
              value={sortOrder}
                ...
                <div className="card-body">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="h-full flex">
                      {!job.assignedAdmin ? (
                        <button 
                          className="btn btn-md rounded-full border-2 border-red text-red gap-2 hover:bg-red hover:text-white"
                          onClick={() => {
                            setSelectedJob(job);
                        ...
                        <div className="flex flex-col gap-4">
                          <div className="rounded-lg flex items-center gap-3 w-full">
                            <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center">
                              <RiUserLine size={24} className="text-gray-600" />
                            </div>
                            <div className="flex flex-col">
                              <span className="text-xs text-muted">Assigned to</span>
                              <span className="font-semibold text-dark">{job.assignedAdmin}</span>
                            </div>
                          </div>
                          ...
                          <div className="dropdown dropdown-hover dropdown-left dropdown-start">
                            <label
                              tabIndex={0}
                              className={`px-6 py-2 text-white rounded-lg w-full flex items-center justify-center gap-2 ${
                                job.status === "approved"
                                  ? "bg-green-500"
                            ...
                            >
                              {job.status === "pending" && <div className="w-2 h-2 rounded-full bg-white animate-pulse" />}
                              {job.status === "pending"
                                ? "Pending"
                                : job.status === "approved"
                                ? "Approved"
                            ...
                            >
                              <li>
                                <button
                                  ...
                                  onClick={() => handleStatusChange(job.id, "changes_requested")}
                                >
                                  <RiMoreLine />
                                  Request Changes
                                </button>
